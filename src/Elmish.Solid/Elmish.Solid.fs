...
type Solid with
    ...
    static member createElmishStore(program: Program<'Arg, 'Model, 'Msg, unit>, projection: 'Model -> 'ViewModel, arg: 'Arg): 'ViewModel * ('Msg -> unit) =
    ...
    /// SolidJS can optimize updates for plain JS objects and arrays in the store, so you can provide a projection
    /// to do this transformation (e.g. convert records into anonymous records and lists into arrays)
    static member createElmishStore(init: 'Arg -> 'Model * Cmd<'Msg>, update: 'Msg -> 'Model -> 'Model * Cmd<'Msg>, projection: 'Model -> 'ViewModel, arg: 'Arg) =
